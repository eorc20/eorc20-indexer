...
import all_miners from "../miners.json"

const queue = new pQueue({ concurrency: 1 });

const writer = fs.createWriteStream("transfers-to-miners.csv");
...
rl.on('line', async (line) => {
    ...
    const tokens = (all_miners as any)[to];
    if ( !tokens ) return;
...
rl.on('close', () => {
  ...
  for ( const tokens of Object.values(all_miners) ) {
  ...
  for ( const [address, {value, multi_transfers}] of sorted) {
    ...
    for ( const miner of [...miners] ) {
      if ( miner in all_miners) {
        tokens += (all_miners as any)[miner];
      }
    }
  ...
  fs.writeFileSync("funders.json", JSON.stringify(current, null, 2));
